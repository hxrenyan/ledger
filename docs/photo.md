...
第一步会发送「完整 OCR」提示和图片 —— 当前默认使用硅基流动上的 `deepseek-ai/DeepSeek-OCR`，
提示要求保留备注、附言、明细和底部小字，并禁止输出 HTML/Markdown 表格或坐标框；服务端还会把模型偶尔返回的 HTML 表格转换成可编辑的制表符文本。结构化交给第二步的语言模型。
